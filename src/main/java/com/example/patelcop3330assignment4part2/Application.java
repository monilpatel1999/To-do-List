package com.example.patelcop3330assignment4part2;
...
class assignments {
    ...
    public static void main(String[] ignored) throws IOException, ClassNotFoundException {
        ...
        assignments.assignments.Tree tree = new assignments.assignments.Tree("/tmp/todos.tree.db");
...
// add_container "id" "title"
                if (args.length < 3){
                    System.out.print("Add a container requires an id and a title");
                    continue;
                }
                int id = Integer.parseInt(args[1]);
                String title = args[2];
                tree.findAndAddTodo(new assignments.assignments.Container(title), id);
                System.out.println("Done adding a container : " + title + " to id: " + id);
            } else if ("add_item".equals(firstArg)) {
// add_item "id" "title"
                if (args.length < 3){
                    System.out.print("Add an item requires a container id and a title");
                    continue;
                }
                int id = Integer.parseInt(args[1]);
                String title = args[2];
                tree.findAndAddTodo(new assignments.assignments.Item(title), id);
                System.out.println("Done adding item: " + title + " to id: " + id);
            } else if ("delete".equals(firstArg)){
// delete "id"
                if (args.length < 2){
                    System.out.print("Delete a container requires a container id");
                    continue;
                }
                int id = Integer.parseInt(args[1]);
                ...
                System.out.print("Unknown argument: " + firstArg);
    ...
    public static class Tree implements Serializable {
         ...
         */
        private final assignments.assignments.Container root = new assignments.assignments.Container("");

        public Tree(String saveLocation){
            this.saveLocation = saveLocation;
        }

        public void findAndAddTodo(assignments.assignments.TodoAbstract todo, int id){
            if (id == 0){
                root.add(todo);
            } else {
        ...
        public Tree findAndDeleteTodo(int id){
            if (id == 0){
                final var printf = System.out.printf("Can't delete the root");
                return this;
            } else {
        ...
        private String recursivePrint(assignments.assignments.TodoAbstract node, int level){
            ...
            result.append(indent).append("-").append(node.getClass().getSimpleName()).append("[id]=").append(node.id).append("\n");
            result.append(indent).append("(todo): ").append(node.title).append("\n");

            if (node.getClass() == assignments.assignments.Container.class){
                for (Map.Entry<Integer, assignments.assignments.TodoAbstract> n : ((assignments.assignments.Container) node).todos.entrySet()){
                    result.append(recursivePrint(n.getValue(), level + 1));
                }
            }
        ...
        public assignments.assignments.Tree load() throws IOException, ClassNotFoundException {
            ...
            try {
                ...
                return (assignments.assignments.Tree) in.readObject();
            } finally {
                try {
                    bis.close();
...
// ignore close exception
    ...
    public static class Container extends assignments.assignments.TodoAbstract {
        public Map<Integer, assignments.assignments.TodoAbstract> todos = new HashMap<>();

        public Container(String title){
            super(title);
        }

        public void add(assignments.assignments.TodoAbstract todo){
        ...
        public void findAndAddContainer(assignments.assignments.TodoAbstract todo, int id){
            if (this.id != id){
                for(Map.Entry<Integer, assignments.assignments.TodoAbstract> n : todos.entrySet()){
                    if (n.getValue().getClass() == assignments.assignments.Container.class){
                        ((assignments.assignments.Container)n.getValue()).findAndAddContainer(todo, id);
                    }
                }
            } else {
                add(todo);
        ...
        public void findAndDeleteTodo(int id) {
            if (todos.containsKey(id)) {
                todos.remove(id);
            } else {
                for (Map.Entry<Integer, assignments.assignments.TodoAbstract> n : todos.entrySet()) {
                    if (n.getValue().getClass() == assignments.assignments.Container.class) {
                        ((assignments.assignments.Container) n.getValue()).findAndDeleteTodo(id);
                    }
                }
            }
    ...
    public static class Item extends assignments.assignments.TodoAbstract {
        public Item(String title) {
            super(title);
        }
    }

    public static abstract class TodoAbstract implements assignments.assignments.Todo, Serializable {
